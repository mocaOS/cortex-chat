...
export default function Header({
  ...
  // New chat without a trip through the sidebar.
  onNewChat?: () => void;
}) {
  useLocale();
      ...
      <div className="flex items-center gap-1">
        {onNewChat && (
          <button
            ...
            className="group relative w-9 h-9 rounded-[var(--radius)] flex items-center justify-center text-[var(--fg2)] hover:text-[var(--fg1)] hover:bg-[var(--muted)] transition-colors cursor-pointer"
          >
            {/* square-pen (compose) icon */}
            <svg
              className="w-5 h-5"
              fill="none"
              viewBox="0 0 24 24"
