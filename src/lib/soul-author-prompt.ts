...
// ---------------------------------------------------------------------------
// Legacy single-prompt path: used only when PERSONALITY_LLM_* is not
// configured — the whole author prompt rides as a deep-research question.
// Works only on backends with prompt security disabled (the injection
// classifier deflects it otherwise).
export function buildSoulAuthorPrompt(opts: {
  ...
  if (previousDraft && refinement) {
    return `You are a SOUL file editor. Apply the following requested changes to the existing SOUL file — the changes must be OBVIOUS in the result. Untouched sections stay as they are.
...
Output ONLY the complete revised SOUL.md.`;
  }
  return `You are a SOUL file author. A SOUL file is a portable identity document for an AI assistant. Research this knowledge base first (domain, concrete names, document types), then write the persona grounded in what you found.
