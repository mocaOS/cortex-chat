...
export function renderEmailLayout(input: EmailLayoutInput): string {
  ...
  const header = input.logoCid
    ? `<img src="cid:${escapeHtml(input.logoCid)}" alt="${title}" height="32" style="height:32px;width:auto;display:block;margin:0 auto;" />`
...
<body style="margin:0;padding:0;background:#0e0e0e;">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0e0e0e;padding:32px 12px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#1a1a1a;border:1px solid #2a2a2a;border-top:3px solid ${accent};border-radius:12px;overflow:hidden;">
...
export function emailButton(label: string, href: string, accentHex: string): string {
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:20px 0;"><tr><td style="border-radius:8px;background:${escapeHtml(accentHex)};">
    <a href="${escapeHtml(href)}" style="display:inline-block;padding:11px 22px;font:600 14px ${FONT};color:#111111;text-decoration:none;border-radius:8px;">${escapeHtml(label)}</a>
  </td></tr></table>`;
}
