...
- **Source maps** are uploaded by `scripts/glitchtip-sourcemaps.mjs`, chained after `next build` (token-gated: no `SENTRY_AUTH_TOKEN` → build succeeds, upload skipped, `.map` files still stripped from `.next/static`). The plugin's own upload is disabled in `next.config.ts` — the instance (GlitchTip **4.0.12**) needs three things the plugin can't do: (1) maps paired strictly by `<file>.js` ↔ `<file>.js.map` **name convention** — Turbopack's client/edge maps use independent hashes, so the script copies each referenced map to the canonical name; (2) `sentry-cli sourcemaps inject` must run on client chunks **before** upload so browsers register debug IDs (`_sentryDebugIds` snippet; forward-compat for GlitchTip ≥ 4.2 debug-id matching — the Node runtime instead reads Turbopack's `//# debugId` comments from disk, so `.next/server` JS is never modified post-build, which would desync the already-assembled standalone output); (3) re-uploading an existing release 500s and can corrupt it — the script skips when the release already has artifacts, so same-commit rebuilds are safe.
- **Releases must be slug-safe** (`[-a-zA-Z0-9_]`): GlitchTip's release routes 403 on `@ . +` in the version. `next.config.ts` derives `cortex-chat-<shortsha>` (`SENTRY_RELEASE` > Coolify's `SOURCE_COMMIT` > local git > package version, sanitized) and injects it into bundles; the script recomputes the same value — keep both in sync. `release.finalize` is off (GlitchTip 405s it).
