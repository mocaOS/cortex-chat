...
- **Soul Builder ("Describe" tab):** `POST /api/me/souls/generate` (SSE) runs ONE deep-research turn whose question is the soul-author prompt (`src/lib/soul-author-prompt.ts` — soulweaver's PRIORITY GUIDANCE pattern, output contract = "write ONLY the SOUL.md" with frontmatter + the 5 shared sections). Grounds the persona in the actual knowledge base; falls back to chat-mode on 4xx; no chat persistence, no memory, **no analytics/soul injection on itself**. Client streams into a preview (`generateSoulStream` in `src/lib/assistants-client.ts`), strips `[src_N]` markers from the draft, supports a refine loop (previous draft + requested changes). Shared composer component: `src/components/souls/SoulComposer.tsx` (user modal + admin page).
